...
@section('content')
    ...
    <div class="report-wrapper">
        <h1 class="h2 text-center">{{ trans(\Locales::getNamespace() . '/datatables.titleReportPoolUsageContractsTracker') }}</h1>

        {!! Form::open(['url' => \Locales::route('reports/pool-usage-contracts-tracker/generate'), 'id' => 'report-form', 'data-ajax-queue' => 'sync', 'data-alert-position' => 'insertAfter', 'class' => 'ajax-lock form-inline', 'role' => 'form']) !!}

        <div class="row-wrapper">
            <div class="form-group ajax-lock">
                {!! Form::label('input-projects', trans(\Locales::getNamespace() . '/forms.projectLabel')) !!}
                {!! Form::multiselect('projects[]', $multiselect['projects'], ['id' => 'input-projects', 'class' => 'form-control', 'multiple' => 'multiple', 'data-ajax-queue' => 'sync', 'data-href' => \Locales::route('reports/pool-usage-contracts-tracker/get-buildings')]) !!}
            </div>
            <div class="form-group ajax-lock">
                {!! Form::label('input-buildings', trans(\Locales::getNamespace() . '/forms.buildingLabel')) !!}
                {!! Form::multiselect('buildings[]', $multiselect['buildings'], ['id' => 'input-buildings', 'class' => 'form-control', 'multiple' => 'multiple', 'data-ajax-queue' => 'sync', 'data-href' => \Locales::route('reports/pool-usage-contracts-tracker/get-apartments')]) !!}
            </div>
        ...
        <div class="row-wrapper">
            ...
            <div class="form-group">
                {!! Form::label('input-puct', trans(\Locales::getNamespace() . '/forms.puctlLabel')) !!}
                {!! Form::multiselect('puct', $multiselect['puct'], ['id' => 'input-puct', 'class' => 'form-control']) !!}
            </div>
        </div>
        <div class="row-wrapper">
            <div class="form-group">
        ...
        <div class="row-wrapper">
            ...
            <div class="btn-group">
                <a id="button-reset-report" href="{{ \Locales::route('reports/pool-usage-contracts-tracker') }}" class="btn btn-danger disabled">
...
@section('script')
    ...
    $('#button-download-report').click(function(e) {
        ...
        unikat.ajaxify({
            ...
            functionParams: ['uuid'],
            function: function(params) {
                window.location.href = '{{ \Locales::route('reports/pool-usage-contracts-tracker/download') }}?uuid=' + params.uuid;
            },
        });
    });
@endsection
