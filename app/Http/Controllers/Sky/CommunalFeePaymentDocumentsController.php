...
use App\Models\Sky\CommunalFeePaymentDocuments;
use App\Http\Requests\Sky\CommunalFeePaymentDocumentsRequest;

class CommunalFeePaymentDocumentsController extends Controller {

    protected $route = 'communal-fees-payment-documents';
    ...
    public function __construct()
    {
        $this->datatables = [
            $this->route => [
                'dom' => 'tr',
                'title' => trans(\Locales::getNamespace() . '/datatables.titleCommunalPaymentDocuments'),
                'url' => \Locales::route($this->route, true),
                'class' => 'table-checkbox table-striped table-bordered table-hover',
                'columns' => [
                    [
                        'selector' => 'communal_fees_payment_documents.id',
                        'id' => 'id',
                        'checkbox' => true,
                        'order' => false,
                    ...
                    [
                        'selector' => 'communal_fees_payment_documents.type',
                        'id' => 'type',
                        'name' => trans(\Locales::getNamespace() . '/datatables.type'),
                        'order' => false,
                        'replace' => [
                            'array' => trans(\Locales::getNamespace() . '/multiselect.communalPaymentDocuments'),
                        ],
                        'class' => 'vertical-center',
                    ],
                    [
                        'selector' => 'communal_fees_payment_documents.file',
                        'id' => 'file',
                        'name' => trans(\Locales::getNamespace() . '/datatables.file'),
                        'order' => false,
                        'class' => 'text-center vertical-center',
                        'file' => [
                            'selector' => ['communal_fees_payment_documents.extension'],
                            'extension' => 'extension',
                            'route' => $this->route . '/download',
                        ],
                    ],
                    [
                        'selector' => 'communal_fees_payment_documents.size',
                        'id' => 'size',
    ...
    public function index(DataTable $datatable, Request $request, $apartment = null, $communalFeesSlug = null, $year = null, $payment = null)
    {
        $breadcrumbs = [];

        $apartment = Apartment::findOrFail($apartment);
        $year = Year::where('year', $year)->firstOrFail();
        $payment = CommunalFeePayment::findOrFail($payment);
        ...
        $datatable->setup(CommunalFeePaymentDocuments::where('communal_fees_payment_id', $payment->id), $this->route, $this->datatables[$this->route]);
    ...
    public function upload(Request $request, FineUploader $uploader, $chunk = null)
    {
        ...
        $payment = CommunalFeePayment::findOrFail($request->input('id'));
        ...
        if (isset($response['success']) && $response['success'] && isset($response['fileName'])) {
            if ($reupload) {
                ...
                $file = CommunalFeePaymentDocuments::findOrFail($request->input('row'));

                Storage::disk('local-public')->deleteDirectory($uploader->uploadDirectory . '/' . $file->uuid);
            } else {
                $file = new CommunalFeePaymentDocuments;
                $file->communal_fees_payment_id = $request->input('id');
            }
            ...
            $response['data'] = [
                'id' => $file->id,
                'type' => $file->type ? trans(\Locales::getNamespace() . '/multiselect.communalPaymentDocuments.' . $file->type) : null,
    ...
    public function destroy(DataTable $datatable, CommunalFeePaymentDocuments $file, Request $request)
    {
        $count = count($request->input('id'));

        $uuids = CommunalFeePaymentDocuments::find($request->input('id'))->lists('communal_fees_payment_id', 'uuid');
        $payment = CommunalFeePayment::findOrFail($uuids->first());

        if ($count > 0 && $file->destroy($request->input('id'))) {
            foreach ($uuids as $uuid => $communal_fees_payment_id) {
            ...
            $datatable->setup(CommunalFeePaymentDocuments::where('communal_fees_payment_id', $communal_fees_payment_id), $request->input('table'), $this->datatables[$request->input('table')], true);
    ...
    public function edit(Request $request, $id = null)
    {
        ...
        $types = array_merge($types, trans(\Locales::getNamespace() . '/multiselect.communalPaymentDocuments'));
    ...
    public function update(DataTable $datatable, CommunalFeePaymentDocumentsRequest $request)
    {
        $file = CommunalFeePaymentDocuments::findOrFail($request->input('id'))->first();
        ...
        if ($file->update($request->all())) {
            ...
            $datatable->setup($file->where('communal_fees_payment_id', $file->communal_fees_payment_id), $request->input('table'), $this->datatables[$request->input('table')], true);
            $datatable->setOption('url', \Locales::route($this->route));
            $datatables = $datatable->getTables();
    ...
    {
        $file = CommunalFeePaymentDocuments::findOrFail($id);
        $apartment = $file->communalFeePayment->apartment_id;
