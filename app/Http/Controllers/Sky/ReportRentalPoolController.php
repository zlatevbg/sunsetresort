...
class ReportRentalPoolController extends Controller
{
    ...
    public function __construct()
    {
        $this->datatables = [
            $this->route => [
                ...
                'url' => \Locales::route('reports/rental-pool'),
    ...
    public function index(DataTable $datatable, Request $request, Apartment $apartments)
    {
        ...
        $rental = [];
        foreach (trans(\Locales::getNamespace() . '/multiselect.reportRentalOptions') as $key => $value) {
            $rental[] = ['id' => $key, 'name' => $value];
        }

        $rental = array_merge($rental, RentalContract::withTrashed()->withTranslation()->select('rental_contract_translations.name', 'rental_contracts.id')->leftJoin('rental_contract_translations', 'rental_contract_translations.rental_contract_id', '=', 'rental_contracts.id')->where('rental_contract_translations.locale', \Locales::getCurrent())->where(\DB::raw('YEAR(contract_dfrom1) + max_duration'), '>=', date('Y'))->orderBy('rental_contract_translations.name')->get()->toArray());
        ...
        if ($request->ajax() || $request->wantsJson()) {
            ...
            return view(\Locales::getNamespace() . '.' . $this->route . '.rental-pool', compact('datatables', 'multiselect'));
        }
    }
    ...
    public function generate(DataTable $datatable, Request $request, Apartment $apartments)
    {
        ...
        }, 'owners' => function ($query) use ($year) {
            if ($year->year < date('Y')) {
                $query->withTrashed();
            }
        }])->join('ownership', 'ownership.apartment_id', '=', 'apartments.id')->join('owners', 'owners.id', '=', 'ownership.owner_id')->whereYear('ownership.created_at', '<=', $year->year)->where(function ($query) use ($year) {
            $query->whereYear('ownership.deleted_at', '>=', $year->year)->orWhereNull('ownership.deleted_at');
        })/*->whereNull('ownership.deleted_at')->whereNull('owners.deleted_at')*/->where('owners.srioc', 0)->groupBy('apartments.id');
        ...
        $selectors = ['apartments.id', 'owners.phone', 'owners.mobile', 'owners.email', 'apartments.building_id', 'project_translations.slug AS projectSlug', 'room_translations.slug AS roomSlug', 'view_translations.slug AS viewSlug'];
    ...
    {
        $columns = ['number', 'options', 'durations', 'owner', 'language', 'room', 'furniture', 'view', 'phone', 'mobile', 'email'];

        $merge = $group ? array_merge($columns, ['project', 'project_id', 'building', 'building_id']) : $columns;
        foreach ($rawData as $key => $value) {
            $data[$key] = collect(array_replace(array_flip($columns), $rawData[$key]->toArray()))->only($merge)->toArray();
        }

        $uuid = (string)\Uuid::generate();
        $filename = 'rental-pool-' . $uuid;
        ...
        array_unshift($data, [
            ...
            'durations' => trans(\Locales::getNamespace() . '/datatables.duration'),
    ...
    {
        return response()->download(storage_path('app/reports/rental-pool-' . $request->input('uuid') . '.xlsx'))->deleteFileAfterSend(true);
    }
}
