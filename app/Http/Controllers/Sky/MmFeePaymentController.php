...
use App\Http\Requests\Sky\MmFeePaymentRequest;

class MmFeePaymentController extends Controller {

    protected $route = 'mm-fees-payments';
            ...
            $this->route => [
                ...
                'joins' => [
                    [
                        ...
                        'constrain' => '=',
                        'foreignColumn' => 'mm_fees_payments.rental_company_id',
                    ...
                    [
                        ...
                        'constrain' => '=',
                        'foreignColumn' => 'mm_fees_payments.owner_id',
                    ],
                ],
                'selectors' => ['mm_fees_payments.comments', 'rental_company_translations.name as company'],
                'columns' => [
                    [
                        'selector' => 'mm_fees_payments.id',
                        'id' => 'id',
                        'checkbox' => true,
                        'order' => false,
                        'class' => 'text-center',
                    ...
                    [
                        'selector' => 'mm_fees_payments.paid_at',
                        'id' => 'paid_at',
                        ...
                        'join' => [
                            ...
                            'constrain' => '=',
                            'foreignColumn' => 'mm_fees_payments.year_id',
                        ],
                        'link' => [
                            'selector' => ['years.year', 'mm_fees_payments.apartment_id'],
                            'icon' => 'folder-open',
                            'route' => 'mm-fees-payment-documents',
                            'routeParametersPrepend' => ['apartment_id' => 'mm-fees', 'year' => '', 'id' => ''],
                        ],
                    ],
                    [
                        'selector' => 'mm_fees_payments.amount',
                        'id' => 'amount',
                    ...
                    [
                        ...
                        'join' => [
                            ...
                            'constrain' => '=',
                            'foreignColumn' => 'mm_fees_payments.payment_method_id',
    ...
    public function index(DataTable $datatable, Request $request, $apartment = null, $mmFeesSlug = null, $year = null)
    {
        $breadcrumbs = [];
        ...
        $datatable->setup(MmFeePayment::where('apartment_id', $apartment->id)->where('year_id', $year->id), $this->route, $this->datatables[$this->route]);
    ...
    public function save(DataTable $datatable, MmFeePaymentRequest $request)
    {
        ...
        $payment = MmFeePayment::create($request->all());
        ...
        if ($payment->id) {
            ...
            $datatable->setup(MmFeePayment::where('apartment_id', $apartment->id)->where('year_id', $year->id), $request->input('table'), $this->datatables[$request->input('table')], true);
    ...
    public function destroy(DataTable $datatable, MmFeePayment $payment, Request $request)
    {
        $apartment = Apartment::findOrFail([$request->input('apartment')])->first();
        $year = Year::findOrFail([$request->input('year')])->first();
    ...
    {
        $payment = MmFeePayment::findOrFail($id);
    ...
    public function update(DataTable $datatable, MmFeePaymentRequest $request)
    {
        $payment = MmFeePayment::findOrFail($request->input('id'))->first();
        ...
        if ($payment->update($request->all())) {
            ...
            $datatable->setup(MmFeePayment::where('apartment_id', $payment->apartment_id)->where('year_id', $payment->year_id), $request->input('table'), $this->datatables[$request->input('table')], true);
            $datatable->setOption('url', \Locales::route($this->route, true));
            $datatables = $datatable->getTables();
