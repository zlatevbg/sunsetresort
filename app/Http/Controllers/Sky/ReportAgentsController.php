...
class ReportAgentsController extends Controller
{
    ...
    public function __construct()
    {
        $this->datatables = [
            $this->route => [
                ...
                'url' => \Locales::route('reports/agents'),
                ...
                'joins' => [
                    ...
                    [
                        'table' => 'agents',
                        'localColumn' => 'agents.id',
                        'constrain' => '=',
                        'foreignColumn' => 'agent_access.agent_id',
                ...
                'columns' => [
                    ...
                    [
                        'selector' => 'agents.name AS agent',
                        'id' => 'agent',
                        'name' => trans(\Locales::getNamespace() . '/datatables.agent'),
                        'class' => 'vertical-center',
    ...
    public function index(DataTable $datatable, Request $request, AgentAccess $agents)
    {
        $datatable->setOption('skipLoading', true, $this->route);
        $datatable->setup($agents, $this->route, $this->datatables[$this->route]);
        ...
        $this->multiselect['agents']['options'] = Agent::distinct()->select('agents.id', 'agents.name')->orderBy('name')->get()->toArray();
        $this->multiselect['agents']['selected'] = '';

        $group = [];
        foreach (trans(\Locales::getNamespace() . '/multiselect.reportGroupByAgents') as $key => $value) {
        ...
        if ($request->ajax() || $request->wantsJson()) {
            ...
            return view(\Locales::getNamespace() . '.' . $this->route . '.agents', compact('datatables', 'multiselect'));
        }
    }
    ...
    public function generate(DataTable $datatable, Request $request, AgentAccess $agents)
    {
        set_time_limit(0);

        $agents = $agents->distinct()->leftJoin('apartments', function ($join) {
             $join->on('apartments.id', '=', 'agent_access.apartment_id')->whereNull('apartments.deleted_at');
        });

        if ($request->input('projects')) {
            $agents = $agents->whereIn('apartments.project_id', $request->input('projects'));
        }

        if ($request->input('buildings')) {
            $agents = $agents->whereIn('apartments.building_id', $request->input('buildings'));
        }

        if ($request->input('apartments')) {
            $agents = $agents->whereIn('apartments.id', $request->input('apartments'));
        }

        if ($request->input('agents')) {
            $agents = $agents->whereIn('agent_access.agent_id', $request->input('agents'));
        }

        $selectors = ['agents.comments'];
        if ($request->input('group') === 'project') {
        ...
        } elseif ($request->input('group') === 'agent') {
            ...
            $agents = $agents->groupBy('agent_access.agent_id');
        } elseif ($request->input('group') === 'apartment') {
            $this->datatables[$this->route]['columns'][0]['selectRaw'] = 'GROUP_CONCAT(DISTINCT agents.name ORDER BY agents.name SEPARATOR ", ") as agent';
            $agents = $agents->groupBy('apartments.id');
        }
        ...
        $datatable->setup($agents, $this->route, $this->datatables[$this->route], true);
        $datatables = $datatable->getTables();
    ...
    {
        $columns = ['apartment', 'agent', 'phone', 'email', 'dfrom', 'dto', 'comments'];
        ...
        array_unshift($data, [
            ...
            'agent' => trans(\Locales::getNamespace() . '/datatables.agent'),
    ...
    {
        return response()->download(storage_path('app/reports/agents-' . $request->input('uuid') . '.xlsx'))->deleteFileAfterSend(true);
    }
}
