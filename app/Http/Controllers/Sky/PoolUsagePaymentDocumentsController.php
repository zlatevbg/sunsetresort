...
use App\Models\Sky\PoolUsagePaymentDocuments;
use App\Http\Requests\Sky\PoolUsagePaymentDocumentsRequest;

class PoolUsagePaymentDocumentsController extends Controller {

    protected $route = 'pool-usage-payment-documents';
    ...
    public function __construct()
    {
        $this->datatables = [
            $this->route => [
                'dom' => 'tr',
                'title' => trans(\Locales::getNamespace() . '/datatables.titlePoolPaymentDocuments'),
                'url' => \Locales::route($this->route, true),
                'class' => 'table-checkbox table-striped table-bordered table-hover',
                'columns' => [
                    [
                        'selector' => 'pool_usage_payment_documents.id',
                        'id' => 'id',
                        'checkbox' => true,
                        'order' => false,
                    ...
                    [
                        'selector' => 'pool_usage_payment_documents.type',
                        'id' => 'type',
                        'name' => trans(\Locales::getNamespace() . '/datatables.type'),
                        'order' => false,
                        'replace' => [
                            'array' => trans(\Locales::getNamespace() . '/multiselect.poolPaymentDocuments'),
                        ],
                        'class' => 'vertical-center',
                    ],
                    [
                        'selector' => 'pool_usage_payment_documents.file',
                        'id' => 'file',
                        'name' => trans(\Locales::getNamespace() . '/datatables.file'),
                        'order' => false,
                        'class' => 'text-center vertical-center',
                        'file' => [
                            'selector' => ['pool_usage_payment_documents.extension'],
                            'extension' => 'extension',
                            'route' => $this->route . '/download',
                        ],
                    ],
                    [
                        'selector' => 'pool_usage_payment_documents.size',
                        'id' => 'size',
    ...
    public function index(DataTable $datatable, Request $request, $apartment = null, $poolUsageSlug = null, $year = null, $payment = null)
    {
        $breadcrumbs = [];

        $apartment = Apartment::findOrFail($apartment);
        $year = Year::where('year', $year)->firstOrFail();
        $payment = PoolUsagePayment::findOrFail($payment);
        ...
        $datatable->setup(PoolUsagePaymentDocuments::where('pool_usage_payment_id', $payment->id), $this->route, $this->datatables[$this->route]);
    ...
    public function upload(Request $request, FineUploader $uploader, $chunk = null)
    {
        ...
        $payment = PoolUsagePayment::findOrFail($request->input('id'));
        ...
        if (isset($response['success']) && $response['success'] && isset($response['fileName'])) {
            if ($reupload) {
                ...
                $file = PoolUsagePaymentDocuments::findOrFail($request->input('row'));

                Storage::disk('local-public')->deleteDirectory($uploader->uploadDirectory . '/' . $file->uuid);
            } else {
                $file = new PoolUsagePaymentDocuments;
                $file->pool_usage_payment_id = $request->input('id');
            }
            ...
            $response['data'] = [
                'id' => $file->id,
                'type' => $file->type ? trans(\Locales::getNamespace() . '/multiselect.poolPaymentDocuments.' . $file->type) : null,
    ...
    public function destroy(DataTable $datatable, PoolUsagePaymentDocuments $file, Request $request)
    {
        $count = count($request->input('id'));

        $uuids = PoolUsagePaymentDocuments::find($request->input('id'))->lists('pool_usage_payment_id', 'uuid');
        $payment = PoolUsagePayment::findOrFail($uuids->first());

        if ($count > 0 && $file->destroy($request->input('id'))) {
            foreach ($uuids as $uuid => $pool_usage_payment_id) {
            ...
            $datatable->setup(PoolUsagePaymentDocuments::where('pool_usage_payment_id', $pool_usage_payment_id), $request->input('table'), $this->datatables[$request->input('table')], true);
    ...
    public function edit(Request $request, $id = null)
    {
        ...
        $types = array_merge($types, trans(\Locales::getNamespace() . '/multiselect.poolPaymentDocuments'));
    ...
    public function update(DataTable $datatable, PoolUsagePaymentDocumentsRequest $request)
    {
        $file = PoolUsagePaymentDocuments::findOrFail($request->input('id'))->first();
        ...
        if ($file->update($request->all())) {
            ...
            $datatable->setup($file->where('pool_usage_payment_id', $file->pool_usage_payment_id), $request->input('table'), $this->datatables[$request->input('table')], true);
            $datatable->setOption('url', \Locales::route($this->route));
            $datatables = $datatable->getTables();
    ...
    {
        $file = PoolUsagePaymentDocuments::findOrFail($id);
        $apartment = $file->poolUsagePayment->apartment_id;
