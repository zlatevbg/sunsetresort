...
class ReportLegalRepresentativesController extends Controller
{
    ...
    public function __construct()
    {
        $this->datatables = [
            $this->route => [
                ...
                'url' => \Locales::route('reports/legal-representatives'),
                ...
                'joins' => [
                    ...
                    [
                        ...
                        'localColumn' => 'legal_representatives.id',
                        'constrain' => '=',
                        'foreignColumn' => 'legal_representative_access.legal_representative_id',
                        'whereNull' => 'legal_representatives.deleted_at',
                    ],
                    [
                ...
                'columns' => [
                    ...
                    [
                        'selector' => 'legal_representatives.name AS legalRepresentative',
                        'id' => 'legalRepresentative',
                        'name' => trans(\Locales::getNamespace() . '/datatables.legalRepresentative'),
                        'class' => 'vertical-center',
    ...
    public function index(DataTable $datatable, Request $request, LegalRepresentativeAccess $legalRepresentatives)
    {
        $datatable->setOption('skipLoading', true, $this->route);
        $datatable->setup($legalRepresentatives, $this->route, $this->datatables[$this->route]);
        ...
        $this->multiselect['representatives']['options'] = LegalRepresentative::distinct()->select('legal_representatives.id', 'legal_representatives.name')->orderBy('name')->get()->toArray();
        $this->multiselect['representatives']['selected'] = '';

        $group = [];
        foreach (trans(\Locales::getNamespace() . '/multiselect.reportGroupByLegalRepresentatives') as $key => $value) {
        ...
        if ($request->ajax() || $request->wantsJson()) {
            ...
            return view(\Locales::getNamespace() . '.' . $this->route . '.legal-representatives', compact('datatables', 'multiselect'));
        }
    }
    ...
    public function generate(DataTable $datatable, Request $request, LegalRepresentativeAccess $legalRepresentatives)
    {
        set_time_limit(0);

        $legalRepresentatives = $legalRepresentatives->distinct()->leftJoin('apartments', function ($join) {
             $join->on('apartments.id', '=', 'legal_representative_access.apartment_id')->whereNull('apartments.deleted_at');
        });

        if ($request->input('projects')) {
            $legalRepresentatives = $legalRepresentatives->whereIn('apartments.project_id', $request->input('projects'));
        }

        if ($request->input('buildings')) {
            $legalRepresentatives = $legalRepresentatives->whereIn('apartments.building_id', $request->input('buildings'));
        }

        if ($request->input('apartments')) {
            $legalRepresentatives = $legalRepresentatives->whereIn('apartments.id', $request->input('apartments'));
        }

        if ($request->input('representatives')) {
            $legalRepresentatives = $legalRepresentatives->whereIn('legal_representative_access.legal_representative_id', $request->input('representatives'));
        }
        ...
        if ($request->input('group') === 'project') {
        ...
        } elseif ($request->input('group') === 'legalRepresentative') {
            ...
            $legalRepresentatives = $legalRepresentatives->groupBy('legal_representative_access.legal_representative_id');
        } elseif ($request->input('group') === 'apartment') {
            $this->datatables[$this->route]['columns'][0]['selectRaw'] = 'GROUP_CONCAT(DISTINCT legal_representatives.name ORDER BY legal_representatives.name SEPARATOR ", ") as legalRepresentative';
            $legalRepresentatives = $legalRepresentatives->groupBy('apartments.id');
        }
        ...
        $datatable->setup($legalRepresentatives, $this->route, $this->datatables[$this->route], true);
        $datatables = $datatable->getTables();
    ...
    {
        $columns = ['apartment', 'legalRepresentative', 'phone', 'email', 'dfrom', 'dto', 'comments'];
        ...
        $filename = 'legal-representatives-' . $uuid;
        ...
        array_unshift($data, [
            ...
            'legalRepresentative' => trans(\Locales::getNamespace() . '/datatables.legalRepresentative'),
            'phone' => trans(\Locales::getNamespace() . '/datatables.phone'),
            'email' => trans(\Locales::getNamespace() . '/datatables.email'),
            'dfrom' => trans(\Locales::getNamespace() . '/datatables.dateFrom'),
            'dto' => trans(\Locales::getNamespace() . '/datatables.dateTo'),
    ...
    {
        return response()->download(storage_path('app/reports/legal-representatives-' . $request->input('uuid') . '.xlsx'))->deleteFileAfterSend(true);
    }
}
