...
use App\Models\Sky\NewsletterAttachmentsOwner;
use App\Models\Sky\Newsletter;
use App\Http\Requests\Sky\NewsletterAttachmentsOwnerRequest;

class NewsletterAttachmentsOwnerController extends Controller {

    protected $route = 'newsletter-attachments-owner';
    ...
    public function __construct()
    {
        $this->datatables = [
            $this->route => [
                'title' => trans(\Locales::getNamespace() . '/datatables.titleAttachmentsOwner'),
                'url' => \Locales::route($this->route, true),
                'class' => 'table-checkbox table-striped table-bordered table-hover',
                'columns' => [
                    [
                        'selector' => 'newsletter_attachments_owner.id',
                        'id' => 'id',
                        'checkbox' => true,
                        'order' => false,
                    ...
                    [
                        'selector' => 'newsletter_attachments_owner.order',
                        'id' => 'order',
                        'order' => false,
                    ...
                    [
                        'selector' => 'newsletter_attachments_owner.file',
                        'id' => 'file',
                        'name' => trans(\Locales::getNamespace() . '/datatables.file'),
                        'order' => false,
                        'class' => 'text-center vertical-center',
                        'file' => [
                            'selector' => ['newsletter_attachments_owner.extension'],
                            'extension' => 'extension',
                    ...
                    [
                        'selector' => 'newsletter_attachments_owner.size',
                        'id' => 'size',
    ...
    public function index(DataTable $datatable, Newsletter $newsletter, Request $request, $id = null)
    {
        ...
        $breadcrumbs[] = ['id' => 'attachments', 'slug' => 'attachments-owner', 'name' => trans(\Locales::getNamespace() . '/multiselect.newsletterProperties.attachments-owner')];
        ...
        $datatable->setup(NewsletterAttachmentsOwner::where('newsletter_id', $newsletter->id), $this->route, $this->datatables[$this->route]);
    ...
    public function upload(Request $request, FineUploader $uploader, $chunk = null)
    {
        ...
        $uploader->uploadDirectory = $this->uploadDirectory . DIRECTORY_SEPARATOR . $request->input('id') . DIRECTORY_SEPARATOR . \Config::get('upload.attachmentsDirectory') . '-owner';
        ...
        if (isset($response['success']) && $response['success'] && isset($response['fileName'])) {
            if ($reupload) {
                ...
                $file = NewsletterAttachmentsOwner::findOrFail($request->input('row'));

                Storage::disk('local-public')->deleteDirectory($uploader->uploadDirectory . '/' . $file->uuid);
            } else {
                $file = new NewsletterAttachmentsOwner;
                $file->order = NewsletterAttachmentsOwner::where('newsletter_id', $request->input('id'))->max('order') + 1;
    ...
    public function destroy(DataTable $datatable, NewsletterAttachmentsOwner $file, Request $request)
    {
        $count = count($request->input('id'));

        $uuids = NewsletterAttachmentsOwner::find($request->input('id'))->lists('newsletter_id', 'uuid');

        if ($count > 0 && $file->destroy($request->input('id'))) {
            foreach ($uuids as $uuid => $newsletter_id) {
                Storage::disk('local-public')->deleteDirectory($this->uploadDirectory . DIRECTORY_SEPARATOR . $newsletter_id . DIRECTORY_SEPARATOR . \Config::get('upload.attachmentsDirectory') . '-owner' . DIRECTORY_SEPARATOR . $uuid);
            }
            ...
            $datatable->setup(NewsletterAttachmentsOwner::where('newsletter_id', $newsletter_id), $request->input('table'), $this->datatables[$request->input('table')], true);
    ...
    {
        $file = NewsletterAttachmentsOwner::findOrFail($id);
    ...
    public function update(DataTable $datatable, NewsletterAttachmentsOwner $files, NewsletterAttachmentsOwnerRequest $request)
    {
        $file = NewsletterAttachmentsOwner::findOrFail($request->input('id'))->first();
    ...
    public function download(Request $request, $id)
    {
        ...
        $uploadDirectory = public_path('upload') . DIRECTORY_SEPARATOR . $this->uploadDirectory . DIRECTORY_SEPARATOR . $file->newsletter_id . DIRECTORY_SEPARATOR . \Config::get('upload.attachmentsDirectory') . '-owner' . DIRECTORY_SEPARATOR . $file->uuid . DIRECTORY_SEPARATOR . $file->file;
