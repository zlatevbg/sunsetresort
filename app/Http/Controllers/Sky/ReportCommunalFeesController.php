...
class ReportCommunalFeesController extends Controller
{
    ...
    public function __construct()
    {
        $this->datatables = [
            $this->route => [
                ...
                'url' => \Locales::route('reports/communal-fees'),
    ...
    public function index(DataTable $datatable, Request $request, Apartment $apartments)
    {
        ...
        $type = [];
        foreach (trans(\Locales::getNamespace() . '/multiselect.reportCommunalFeesType') as $key => $value) {
        ...
        if ($request->ajax() || $request->wantsJson()) {
            ...
            return view(\Locales::getNamespace() . '.' . $this->route . '.communal-fees', compact('datatables', 'multiselect'));
        }
    }
    ...
    public function generate(DataTable $datatable, Request $request, Apartment $apartments)
    {
        ...
        $apartments = $apartments->with(['communalFeesPayments', 'contracts', 'contracts.contractYears' => function ($query) use ($year) {
            $query->where('contract_years.year', $year->year);
        }, 'contracts.rentalContract' => function ($query) {
            $query->withTrashed();
        }])->join('ownership', 'ownership.apartment_id', '=', 'apartments.id')->join('owners', 'owners.id', '=', 'ownership.owner_id')->whereNull('ownership.deleted_at')->whereNull('owners.deleted_at')->groupBy('apartments.id');
        ...
        $selectors = ['apartments.id', 'apartments.building_id', 'owners.phone', 'owners.mobile', 'owners.email'];
    ...
    {
        $columns = ['number', 'amount', 'owner', 'language', 'room', 'furniture', 'view', 'phone', 'mobile', 'email'];

        $merge = $group ? array_merge($columns, ['project', 'project_id1', 'building', 'building_id1']) : $columns;
        foreach ($rawData as $key => $value) {
            $data[$key] = collect(array_replace(array_flip($columns), $rawData[$key]->toArray()))->only($merge)->map(function ($item, $key) {
                return $key === 'amount' ? (float)str_replace(',', '', $item) : $item;
            })->toArray();
        }

        $uuid = (string)\Uuid::generate();
        $filename = 'communal-fees-' . $uuid;
    ...
    {
        return response()->download(storage_path('app/reports/communal-fees-' . $request->input('uuid') . '.xlsx'))->deleteFileAfterSend(true);
    }
}
