...
class ReportPoolUsageContractTrackerController extends Controller
{
    ...
    public function __construct()
    {
        $this->datatables = [
            $this->route => [
                ...
                'url' => \Locales::route('reports/pool-usage-contracts-tracker'),
    ...
    public function index(DataTable $datatable, Request $request, Apartment $apartments)
    {
        ...
        $puct = [];
        foreach (trans(\Locales::getNamespace() . '/multiselect.reportPUCT') as $key => $value) {
            $puct[] = ['id' => $key, 'name' => $value];
        }

        $this->multiselect['puct']['options'] = $puct;
        $this->multiselect['puct']['selected'] = '';
        ...
        if ($request->ajax() || $request->wantsJson()) {
            ...
            return view(\Locales::getNamespace() . '.' . $this->route . '.pool-usage-contract-tracker', compact('datatables', 'multiselect'));
        }
    }
    ...
    public function generate(DataTable $datatable, Request $request, PoolUsageContractTracker $puct)
    {
        set_time_limit(0);

        $puct = $puct->leftJoin('apartments', 'apartments.id', '=', 'pool_usage_contracts_tracker.apartment_id')->join('ownership', 'ownership.apartment_id', '=', 'apartments.id')->join('owners', 'owners.id', '=', 'ownership.owner_id')->leftJoin('years', 'years.id', '=', 'pool_usage_contracts_tracker.year_id')->whereNull('ownership.deleted_at')->whereNull('owners.deleted_at')->where('years.year', '=', $request->input('year'))->groupBy('apartments.id');

        if ($request->has('puct')) {
            if ($request->input('puct') == 'no-contract') {
                $puct = $puct->where('pool_usage_contracts_tracker.is_active', 0);
            } elseif ($request->input('puct') == 'contract') {
                $puct = $puct->where('pool_usage_contracts_tracker.is_active', 1);
            }
        }

        if ($request->input('projects')) {
            $puct = $puct->whereIn('apartments.project_id', $request->input('projects'));
        }

        if ($request->input('buildings')) {
            $puct = $puct->whereIn('apartments.building_id', $request->input('buildings'));
        }

        if ($request->input('apartments')) {
            $puct = $puct->whereIn('apartments.id', $request->input('apartments'));
        }

        if ($request->input('rooms')) {
            $puct = $puct->whereIn('apartments.room_id', $request->input('rooms'));
        }

        if ($request->input('furniture')) {
            $puct = $puct->whereIn('apartments.furniture_id', $request->input('furniture'));
        }

        if ($request->input('views')) {
            $puct = $puct->whereIn('apartments.view_id', $request->input('views'));
        }

        if ($request->input('languages')) {
            $puct = $puct->whereIn('owners.locale_id', $request->input('languages'));
        }
        ...
        $datatable->setup($puct, $this->route, $this->datatables[$this->route], true);
        $datatables = $datatable->getTables();
    ...
    public function generateExcel($total, $rawData, $projects, $buildings, $group)
    {
        ...
        $filename = 'pool-usage-contracts-tracker-' . $uuid;
    ...
    {
        return response()->download(storage_path('app/reports/pool-usage-contracts-tracker-' . $request->input('uuid') . '.xlsx'))->deleteFileAfterSend(true);
    }
}
