...
class ReportPoolUsageController extends Controller
{
    ...
    public function __construct()
    {
        $this->datatables = [
            $this->route => [
                ...
                'url' => \Locales::route('reports/pool-usage'),
    ...
    public function index(DataTable $datatable, Request $request, Apartment $apartments)
    {
        ...
        $type = [];
        foreach (trans(\Locales::getNamespace() . '/multiselect.reportPoolUsageType') as $key => $value) {
        ...
        if ($request->ajax() || $request->wantsJson()) {
            ...
            return view(\Locales::getNamespace() . '.' . $this->route . '.pool-usage', compact('datatables', 'multiselect'));
        }
    }
    ...
    public function generate(DataTable $datatable, Request $request, Apartment $apartments)
    {
        ...
        $apartments = $apartments->with(['poolUsagePayments', 'poolUsageContracts', 'contracts', 'contracts.contractYears' => function ($query) use ($year) {
    ...
    {
        return response()->download(storage_path('app/reports/pool-usage-' . $request->input('uuid') . '.xlsx'))->deleteFileAfterSend(true);
    }
}
